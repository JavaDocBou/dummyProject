(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('agiflexApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-agiflexApp-error');
                var entityKey = httpResponse.headers('X-agiflexApp-params');
                if (errorHeader) {
